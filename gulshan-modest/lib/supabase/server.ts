...
import { devMocksEnabled } from '@/lib/customer-session'
...
export async function createClient() {
  ...
  const customSession = await verifyCustomerToken(
    cookieStore.get(CUSTOMER_SESSION_COOKIE)?.value
  )
  const allowMocks = devMocksEnabled()

  const getCustomAuth = (realAuth: any = null) => ({
    getUser: async () => {
      if (customSession) {
        return { data: { user: { id: customSession.sub, email: customSession.email, user_metadata: { full_name: customSession.full_name, role: customSession.role } } }, error: null }
      }
      const hasMockCookie = allowMocks && cookieStore.get('mock-admin-logged-in')?.value === 'true'
      if (hasMockCookie) {
        return { data: { user: { id: 'mock-admin-id', email: 'admin@panjatanayurveda.com', user_metadata: { role: 'admin' } } }, error: null }
      }
    ...
    getSession: async () => {
      if (customSession) {
        return { data: { session: { user: { id: customSession.sub, email: customSession.email, user_metadata: { full_name: customSession.full_name, role: customSession.role } } } }, error: null }
      }
      const hasMockCookie = allowMocks && cookieStore.get('mock-admin-logged-in')?.value === 'true'
      if (hasMockCookie) {
        return { data: { session: { user: { id: 'mock-admin-id', email: 'admin@panjatanayurveda.com', user_metadata: { role: 'admin' } } } }, error: null }
      }
    ...
    signInWithPassword: async (credentials: any) => {
      ...
      if (allowMocks && credentials?.email?.includes('admin')) {
        cookieStore.set('mock-admin-logged-in', 'true', {
          httpOnly: true,
          sameSite: 'lax',
          secure: false,
          path: '/',
    ...
    signOut: async () => {
      ...
      if (allowMocks) {
        cookieStore.set('mock-admin-logged-in', '', {
          httpOnly: true,
          sameSite: 'lax',
          path: '/',
          expires: new Date(0),
  ...
  if (!url || !key || url.includes('placeholder') || url === '') {
    if (!allowMocks) {
      throw new Error('Supabase is not configured and ENABLE_DEV_MOCKS is not true')
    }
    return {
      auth: getCustomAuth(),
    ...
    key,
    {
      cookies: {
        getAll() {
          ...
          return cookieStore.getAll().filter(c => c.name !== CUSTOMER_SESSION_COOKIE)
        },
        setAll(cookiesToSet) {
          try {
